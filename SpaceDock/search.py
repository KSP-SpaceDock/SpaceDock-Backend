...
class Search:
    ...
    def weigh_result(self, result, terms):
    ...
    def search_mods(self, game_id, text, page, limit):
        terms = text.split(' ')
        query = self.db.query(Mod).join(Mod.user).join(Mod.versions).join(Mod.game)
        filters = list()
        for term in terms:
    ...
    def typeahead_mods(self, text):
        query = self.db.query(Mod)
        filters = list()
