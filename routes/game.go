...
func add_game(ctx *iris.Context) {
    ...
    app.Database.Where("id = ?", pubid).First(publisher)
    ...
    game := &objects.Game{}
    app.Database.Where("short = ?", short).First(game)
    if game.Short == short {
        errors = append(errors, "The gameshort already exists.")
        codes = append(codes, 2015)
    }
    app.Database.Where("name = ?", name).First(game)
    if game.Name == name {
...
func game_version_remove(ctx *iris.Context) {
    ...
    app.Database.Where("friendly_version = ?", friendly_version).Where("game_id = ?", game.ID).First(version)
...
func game_version_show(ctx *iris.Context) {
    ...
    version := &objects.GameVersion{}
    app.Database.Where("friendly_version = ?", friendly_version).Or("id = ?", cast.ToUint(friendly_version)).Where("game_id = ?", game.ID).First(version)
