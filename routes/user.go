...
func checkUsernameForRegistration(username string) string {
    ...
    if app.Database.Where("username = ?", username).First(&user); user.Username != "" {
...
func checkEmailForRegistration(email string) string {
    ...
    } else if app.Database.Where("email = ?", email).First(&user); user.Username != "" {
