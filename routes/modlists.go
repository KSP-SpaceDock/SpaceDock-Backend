...
func list_info(ctx *iris.Context) {
    ...
    app.Database.Where("id = ?", listid).First(modlist)
...
func lists_add(ctx *iris.Context) {
    ...
    modlist := &objects.ModList{}
    app.Database.Where("name = ?", name).First(modlist)
...
func list_add_mod(ctx *iris.Context) {
    ...
    app.Database.Where("mod_list_id = ?", modlist.ID).Where("mod_id = ?", mod.ID).First(entry)
